...
describe("<SemesterSwitcher />", () => {
  test("simulates click events", () => {
    ...
    const wrapper = shallow(
      <SemesterSwitcher semester={2} onSelectSemester={onButtonClick} />,
    );
  ...
  describe("semester switching buttons are disabled appropriately", () => {
    test("left button", () => {
      const onButtonClick = jest.fn();
      const wrapper = shallow(
        <SemesterSwitcher semester={1} onSelectSemester={onButtonClick} />,
      );
      const buttons = wrapper.find("button");
      buttons.at(0).simulate("click");
    ...
    test("right button", () => {
      const onButtonClick = jest.fn();
      const wrapper = shallow(
        <SemesterSwitcher semester={4} onSelectSemester={onButtonClick} />,
      );
      const buttons = wrapper.find("button");
      buttons.at(1).simulate("click");
      expect(onButtonClick).not.toBeCalled();
