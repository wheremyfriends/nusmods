...
function make(
  ...
  store.dispatch({
    type: SUCCESS_KEY(FETCH_MODULE_LIST),
    payload: modulesList,
  });
...
describe(TimetableContainerComponent, () => {
  ...
  test("should redirect to activeSemester when semester is empty", async () => {
    const semesters: Semester[] = Object.keys(config.shortSemesterNames).map(
      Number,
    );
    ...
    for (const semester of semesters) {
      const { history } = make("/timetable", {
        app: { activeSemester: semester },
      });
      // eslint-disable-next-line no-await-in-loop
      await waitFor(() =>
        expect(history.location.pathname).toBe(timetablePage(semester)),
      );
    }
  });
  ...
  test("should eventually display imported timetable if there is one", async () => {
    ...
    expect(
      screen.getByText(/Personal Development & Career Management/),
    ).toBeInTheDocument();
  ...
  test("should eventually display imported timetable without any modules loaded", async () => {
    const semester = 1;
    const importedTimetable = {
      [moduleCodeThatCanBeLoaded]: { "Sectional Teaching": "A1" },
    };
    const location = timetableShare(semester, importedTimetable, [
