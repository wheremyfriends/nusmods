...
function parseBrowser(browser: string): {
  platform?: Platform;
  browser: string;
} {
  ...
  if (browser === "samsung")
    return { platform: "mobile", browser: "samsung_internet" };
...
function parseVersion(version: string): number {
  const versions = version
    .split(/-/g)
    .map((versionStr) => parseFloat(versionStr));
...
export function browserlistToBowser(
  browserlist: string[],
): Bowser.Parser.checkTree {
  ...
  browserlist.forEach((browserlistItem) => {
    ...
    if (platform != null) {
      ...
      if (platform === "mobile")
        setMinVersion(minVersions.tablet, browser, minVersion);
      if (platform === "tablet")
        setMinVersion(minVersions.mobile, browser, minVersion);
    } else {
