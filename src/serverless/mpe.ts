...
export const createSubmission = async (
  ...
  if (!validatePreferences(submission.preferences)) {
    throw new Error(
      "Submission preferences failed to meet validation requirements",
    );
  }
  ...
  const validatedSubmission: MpeSubmission = {
    ...
    preferences: submission.preferences.map(
      ({ moduleCode, moduleType }, index) => ({
        rank: index + 1,
        moduleCode,
        moduleType,
      }),
...
const convertToMpeSubmission = (
  submission: unknown,
): MpeSubmission | undefined => {
...
const validatePreferences = (preferences: MpePreference[]): boolean =>
  preferences.every(
    (preference) =>
      !isEmptyString(preference.moduleCode) &&
      isBetweenRangeString(preference.moduleCode, 1, 18),
  );
