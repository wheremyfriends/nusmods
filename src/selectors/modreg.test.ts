...
describe(getRounds, () => {
  ...
  const mockTime = (now: Date | string) => {
    (
      forceTimer as unknown as jest.MockInstance<Date | null, []>
    ).mockReturnValue(new Date(now));
  };
  ...
  test("should return rounds from of the correct type", () => {
    ...
    expect(
      getRounds(settings({ scheduleType: "Graduate" }), DEFAULT_SCHEDULE),
    ).toEqual([]);
  });
  ...
  test("should not return dismissed rounds", () => {
    ...
    expect(
      getRounds(
        settings({ dismissed: [{ type: "Select Courses", name: "1" }] }),
        DEFAULT_SCHEDULE,
      ),
    ).toEqual([
      expect.objectContaining({
