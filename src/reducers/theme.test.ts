...
const themeWithFirstTheme: ThemeState = {
  ...themeInitialState,
  id: themeIds[0],
};
const themeWithLastTheme: ThemeState = {
  ...themeInitialState,
  id: themeIds[themeIds.length - 1],
};
...
describe("theme", () => {
  ...
  describe("can cycle theme", () => {
    test("can cycle forwards and backwards", () => {
      ...
      const nextState: ThemeState = reducer(
        themeInitialState,
        cycleForwardAction,
      );
      expect(themeIds.indexOf(nextState.id)).toEqual(
        themeIds.indexOf(themeInitialState.id) + 1,
      );
    ...
    test("cycle wrapping behavior", () => {
      ...
      const cycleBackwardAction = actions.cycleTheme(-1);
      const nextState2: ThemeState = reducer(
        themeWithFirstTheme,
        cycleBackwardAction,
      );
