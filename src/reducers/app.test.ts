...
const appHasSemesterTwoState: AppState = {
  ...appInitialState,
  activeSemester: anotherSemester,
};
const appHasActiveLessonState: AppState = {
  ...appInitialState,
  activeLesson: lesson,
};
...
test("app should cancel and unset active lesson", () => {
  const nextState: AppState = reducer(
    appHasActiveLessonState,
    cancelModifyLesson(),
  );
...
test("app should subscribe to online status action", () => {
  ...
  expect(reducer(nextState, setOnlineStatus(true))).toHaveProperty(
    "isOnline",
    true,
  );
});
...
describe("notification reducers", () => {
  ...
  test("allow new priority notification to trump existing ones", () => {
    ...
    state = reducer(
      state,
      openNotification(pNotif3.message, { priority: pNotif3.priority }),
    );
    expect(state.notifications).toHaveLength(2);
