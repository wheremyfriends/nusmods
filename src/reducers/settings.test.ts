...
const settingsWithNewStudent: SettingsState = {
  ...initialState,
  newStudent: true,
};
...
const settingsWithDismissedNotifications: SettingsState = produce(
  initialState,
  (draft) => {
...
describe("modRegNotification settings", () => {
  ...
  test("dismiss ModReg notification", () => {
    ...
    expect(
      reducer(
        settingsWithDismissedNotifications,
        actions.dismissModregNotification({
          type: "Select Courses",
          name: "1",
        } as RegPeriod),
  ...
  test("dismiss/enable notifications should be opposites", () => {
    expect(
      reducer(
        ...
        actions.enableModRegNotification({
          type: "Select Courses",
          name: "1",
        } as RegPeriod),
      ).modRegNotification.dismissed,
  ...
  test("toggle ModReg notification globally", () => {
    const state1 = reducer(
      initialState,
      actions.toggleModRegNotificationGlobally(true),
    );
    expect(state1.modRegNotification.enabled).toEqual(true);

    const state2 = reducer(
      initialState,
      actions.toggleModRegNotificationGlobally(false),
    );
  ...
  test("set schedule type", () => {
    expect(
      reducer(initialState, actions.setModRegScheduleType("Graduate"))
        .modRegNotification.scheduleType,
    ).toEqual("Graduate");
    expect(
      reducer(initialState, actions.setModRegScheduleType("Undergraduate"))
        .modRegNotification.scheduleType,
