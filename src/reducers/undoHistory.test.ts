...
const newFSA = (type: string, payload: Record<string, any> = {}) =>
  ({ type, payload }) as Actions;
...
describe("#computeUndoStacks()", () => {
  test("should ignore irrelevant actions", () => {
    ...
    const hist1 = computeUndoStacks(
      hist0,
      newFSA(IGNORED_ACTION),
      state,
      mutatedState,
      config,
    );
  ...
  test("should store data when relevant actions dispatched", () => {
    ...
    const hist1 = computeUndoStacks(
      hist0,
      newFSA(WATCHED_ACTION),
      state,
      mutatedState,
      config,
    );
    const hist2 = computeUndoStacks(
      hist1,
      newFSA(WATCHED_ACTION),
      mutatedState,
      state,
  ...
  test("should redo", () => {
    ...
    const hist2 = computeUndoStacks(
      ...
      mutatedState,
      config,
    ); // Populate present and future, clears past
    ...
    const redoneNoPresent = computeUndoStacks(
      noPresent,
      redo(),
      state,
      state,
  ...
  test("should enforce limit if present", () => {
    ...
    const hist0 = state.undoHistory; // Empty past, present and future
    const hist10 = computeUndoStacks(
      hist0,
      newFSA(WATCHED_ACTION),
      state,
      mutatedState,
    ...
    const hist11 = computeUndoStacks(
      hist10,
      newFSA(WATCHED_ACTION),
      mutatedState,
      state,
    ...
    // Expect limit = 0 to be enforced
    const hist00 = computeUndoStacks(
      hist0,
      newFSA(WATCHED_ACTION),
      state,
      mutatedState,
    ...
    const histN0 = computeUndoStacks(
      hist0,
      newFSA(WATCHED_ACTION),
      state,
      mutatedState,
