...
describe(highlight, () => {
  const h = (...args: Parameters<typeof highlight>) =>
    render(<p>{highlight(...args)}</p>);
  ...
  test("should wrap search terms with <mark>", () => {
    ...
    expect(h("Bar barring barred barn baaa", "bar").find("mark")).toHaveLength(
      4,
    );
  ...
  test("should wrap each of the multiple search terms with <mark>", () => {
    ...
    expect(h(shakespeare, ["some", "born", "great"]).find("mark")).toHaveLength(
      7,
    );
  });
});
...
describe("wrapComponentName()", () => {
  ...
  FunctionalComponentWithDisplayName.displayName =
    "FunctionalComponentDisplayName";

  test("should infer component name from provided component", () => {
    expect(wrapComponentName(TestComponent, "wrapper")).toEqual(
      "wrapper(TestComponent)",
    );
    expect(wrapComponentName(TestPureComponent, "wrapper")).toEqual(
      "wrapper(TestPureComponent)",
    );
    ...
    expect(
      wrapComponentName(FunctionalComponentWithDisplayName, "wrapper"),
