...
test(
  ...
  () => {
    const otherLesson: Lesson = lessonWithDifferentProperty(
      mockLesson,
      "startTime",
      "0000",
    );
    const otherLesson2: Lesson = lessonWithDifferentProperty(
      otherLesson,
      "endTime",
      "2300",
    );
...
test("formatExamDate should format an exam date string correctly", () => {
  expect(formatExamDate("2016-11-23T01:00:00.000Z")).toBe(
    "23-Nov-2016 9:00 AM",
  );
  expect(formatExamDate("2016-01-23T01:00:00.000Z")).toBe(
    "23-Jan-2016 9:00 AM",
  );
  expect(formatExamDate("2016-11-03T01:00:00.000Z")).toBe(
    "03-Nov-2016 9:00 AM",
  );
  expect(formatExamDate("2016-11-03T11:00:00.000Z")).toBe(
    "03-Nov-2016 7:00 PM",
  );
  expect(formatExamDate("2016-11-03T11:30:00.000Z")).toBe(
    "03-Nov-2016 7:30 PM",
  );
  expect(formatExamDate("2016-11-03T00:30:00.000Z")).toBe(
    "03-Nov-2016 8:30 AM",
  );
  expect(formatExamDate("2016-01-03T00:01:00.000Z")).toBe(
    "03-Jan-2016 8:01 AM",
  );
});
...
describe(getFirstAvailableSemester, () => {
  ...
  test("should return the current semester if it is available", () => {
    ...
    expect(
      getFirstAvailableSemester([sem2Data, sem3Data, sem1Data], 1),
    ).toEqual(1);

    expect(getFirstAvailableSemester([sem2Data], 2)).toEqual(2);
    expect(
      getFirstAvailableSemester([sem1Data, sem2Data, sem3Data], 2),
    ).toEqual(2);
  });
