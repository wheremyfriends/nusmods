...
describe(getNewColor, () => {
  ...
  test("it should get random color", () => {
    ...
    function expectValidIndex(
      unexpectedColors: ColorIndex[],
      currentColors: ColorIndex[],
    ) {
      expect(
        without(range(NUM_DIFFERENT_COLORS), ...unexpectedColors),
...
describe(colorLessonsByKey, () => {
  ...
  test("it should assign colors deterministically", () => {
    ...
    range(NUM_DIFFERENT_COLORS * 2).forEach((i) => {
      ...
      expect(coloredLesson).toHaveProperty(
        "colorIndex",
        index % NUM_DIFFERENT_COLORS,
      );
    });
  });
});
...
describe(fillColorMapping, () => {
  test("should return color map with colors for all modules", () => {
    expect(
      Object.keys(fillColorMapping({ CS1010S: {}, CS3216: {} }, {})),
    ).toEqual(["CS1010S", "CS3216"]);

    expect(
      fillColorMapping({ CS1010S: {}, CS3216: {} }, { CS1010S: 0, CS3216: 1 }),
    ).toEqual({
      CS1010S: 0,
      CS3216: 1,
    });
    ...
    expect(
      fillColorMapping({ CS1010S: {}, CS3216: {} }, { CS1010S: 0, CS3216: 0 }),
    ).toEqual({
      CS1010S: 0,
      CS3216: 0,
    });
  ...
  test("should not repeat colors unnecessarily", () => {
    ...
    expect(
      uniqueColors(FILLED_TIMETABLE, { CS3216: 1, CS1101S: 0 }),
    ).toHaveLength(8);
    expect(
      uniqueColors(
        { ...FILLED_TIMETABLE, CS1231: {} },
