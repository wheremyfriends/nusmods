...
describe("searchVenue()", () => {
  ...
  test("should find getVenues based on search", () => {
    ...
    expect(
      searchVenue(venues, "GAMELAN Instrument Room (Studio)", venueLocations),
    ).toEqual(getVenues("AS2-0201"));

    // Case insensitivity
    expect(searchVenue(venues, "lt17")).toEqual(getVenues("LT17"));

    expect(searchVenue(venues, "gamelan instrument room (studio)")).toEqual(
      getVenues("AS2-0201"),
    );
    ...
    expect(searchVenue(venues, "0")).toEqual(
      getVenues("AS6-0333", "S11-0302", "CQT/SR0622"),
    );
...
describe(filterAvailability, () => {
  ...
  test("should not return venues that are excluded from 'find free rooms' feature", () => {
    ...
    expect(
      availableVenues.some((allVenues) => allVenues[0] === "AS6-0333"),
    ).toBe(false);
  });
});
