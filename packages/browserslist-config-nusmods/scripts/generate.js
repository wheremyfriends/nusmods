...
const BROWSER_REGEX =
  /browserCode==(?<code>\w+);browserVersion==(?<version>\S+)?/;
function process(rawData) {
  const total = rawData.reduce(
    (count, row) => count + row.sum_daily_nb_uniq_visitors,
    0,
  );
  ...
  for (const row of rawData) {
    ...
    if (!versionMapping) {
      console.warn(
        `Ignoring browser ${code}'s version ${version} with usage of ${usage}%`,
      );
      continue;
    }
...
async function output(stats) {
  ...
  return Promise.all([
    fs.outputFile(
      pathAtRoot("index.js"),
      `module.exports = ${browserslistString}`,
    ),
    fs.outputJson(pathAtRoot("browserslist-stats.json"), stats, {
      spaces: "\t",
    }),
  ]);
}
