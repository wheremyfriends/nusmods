...
describe("getAcadWeekName", () => {
  it("determines the correct semester for supported weeks", () => {
    expect(getAcadWeekName(1)).toEqual({
      weekType: "Instructional",
      weekNumber: 1,
    });
    expect(getAcadWeekName(3)).toEqual({
      weekType: "Instructional",
      weekNumber: 3,
    });
    expect(getAcadWeekName(7)).toEqual({
      weekType: "Recess",
      weekNumber: null,
    });
    expect(getAcadWeekName(8)).toEqual({
      weekType: "Instructional",
      weekNumber: 7,
    });
    expect(getAcadWeekName(11)).toEqual({
      weekType: "Instructional",
      weekNumber: 10,
    });
    expect(getAcadWeekName(15)).toEqual({
      weekType: "Reading",
      weekNumber: null,
    });
    expect(getAcadWeekName(16)).toEqual({
      weekType: "Examination",
      weekNumber: 1,
    });
    expect(getAcadWeekName(17)).toEqual({
      weekType: "Examination",
      weekNumber: 2,
    });
  });
